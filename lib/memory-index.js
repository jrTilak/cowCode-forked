...
/** Max chunks per embedding API request to avoid timeouts and rate limits. */
...
export async function indexFilesystem(config, options = {}) {
  ...
  const batchSize = Math.max(1, Math.min(500, Number(options.embedBatchSize) || FILESYSTEM_EMBED_BATCH_SIZE));
  let dims = null;
  let dbWithVec = null;
  ...
  for (let b = 0; b < totalBatches; b++) {
    ...
    if (onDir || totalBatches > 1) {
      console.log('[index] Embedding batch', b + 1, '/', totalBatches, '(' + batch.length, 'chunks)...');
    }
    let vectors;
    try {
