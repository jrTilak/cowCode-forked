...
export async function sync(config) {
  ...
  for (const { relPath, mtimeMs } of toProcess) {
    ...
    try {
      ...
      console.error('[memory] embed failed:', err.message);
      throw err;
    }
...
export async function indexChatExchange(config, exchange) {
  ...
  try {
    ...
    console.error('[memory] chat log append failed:', err.message);
    return;
  }
  ...
  try {
    ...
    console.error('[memory] chat embed failed:', err.message);
    return;
  }
  const vec = vectors[0];
