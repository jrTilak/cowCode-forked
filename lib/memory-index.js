...
const DEFAULT_FILESYSTEM_EXCLUDE_DIRS = ['.git', 'node_modules', '.cowcode', '__pycache__', '.venv', 'venv', '.next', '.cache', '.tox', 'dist', 'build'];
 ...
 * @param {{ root?: string, maxDepth?: number, maxChunks?: number, excludeDirs?: string[], onDir?: (dirPath: string) => void }} [options]
 */
export async function indexFilesystem(config, options = {}) {
  ...
  const chunks = walkFilesystemChunks(resolvedRoot, { maxDepth, excludeDirs, onDir, maxChunks }, relPrefix);
  if (chunks.length === 0) {
  ...
  const texts = chunks.map((c) => c.text);
  let vectors;
  if (onDir) console.log('[index] Embedding', chunks.length, 'chunks...');
  try {
    vectors = await embed(texts, config.embedding);
  } catch (err) {
  ...
  if (vectors.length !== chunks.length) throw new Error('Embed count mismatch');
  const dims = vectors[0].length;
  const dbWithVec = getDb(config.indexPath, dims);
  try {
  ...
  const insertChunk = dbWithVec.prepare(
    `INSERT INTO ${CHUNKS_TABLE} (path, start_line, end_line, text, source, chunk_date) VALUES (?, 1, 1, ?, ?, NULL)`
  );
  const insertVec = dbWithVec.prepare(`INSERT INTO ${VECTOR_TABLE} (chunk_id, embedding) VALUES (CAST(? AS INTEGER), ?)`);
  const insertFile = dbWithVec.prepare(`INSERT OR REPLACE INTO ${FILES_TABLE} (path, mtime_ms, source) VALUES (?, ?, ?)`);
  ...
  console.log('[index] Indexed', chunks.length, 'directory chunks from', resolvedRoot);
}
...
async function rebuildVecTable(db, dimensions, embeddingConfig) {
  ...
  const vectors = await embed(texts, embeddingConfig);
