...
export async function handleTelegramGroupMessage(msg, ctx) {
  ...
  // Group guard: rate limit and block cron/scan for non-owners.
  ...
  if (ownerCfg.telegramUserId && !ctx.isOwner(msg.from?.id)) {
    ...
    if (isOverRateLimit(rateKey)) {
      await ctx.bot.sendMessage(chatId, 'Too many requests from this group. Please wait a minute or ask the bot owner.').catch(() => ctx.addPendingTelegram(jidKey, 'Too many requests from this group. Please wait a minute or ask the bot owner.'));
      return;
    }
    if (isCronIntent(text)) {
      await ctx.bot.sendMessage(chatId, 'Cron jobs are not allowed for group members.').catch(() => ctx.addPendingTelegram(jidKey, 'Cron jobs are not allowed for group members.'));
      return;
    }
    if (isScanIntent(text)) {
      await ctx.bot.sendMessage(chatId, 'Scanning is not allowed for group members.').catch(() => ctx.addPendingTelegram(jidKey, 'Scanning is not allowed for group members.'));
      return;
    }
